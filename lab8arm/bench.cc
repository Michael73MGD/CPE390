...
void benchmark1(const char msg[], Func f, uint32_t n) {
	for (uint32_t trials = 0; trials < 5; trials++) {
		...
		uint64_t res = f(n);
		clock_t t1 = clock();
		cout << setw(12) << msg << setw(18) << res << '\t' << (double)(t1-t0)/CLOCKS_PER_SEC << '\n';
	}
}
   ...
   */
   ...
   benchmark1("a2", a2, n);                                                  
	...
	const uint32_t numTrials = 100;
	 benchmark2("readOneLocation", readOneLocation, narray, numTrials);              	
	 benchmark2("readArray", readArray, narray, numTrials);                        
  	 benchmark2("writeArray", writeArray, narray, numTrials);
